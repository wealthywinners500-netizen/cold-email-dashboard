...
export type Tab = 'all' | 'warm-up' | 'hot-leads' | 'spam';

export const VALID_TABS: readonly Tab[] = ['all', 'warm-up', 'hot-leads', 'spam'];

export const HOT_LEAD_CLASSIFICATIONS = ['OBJECTION', 'INTERESTED'] as const;
...
// Conservative AND per Dean's lock (design doc §5):
...
export function isHotLeadThread(thread: ThreadLike, knownSenders: Set<string>): boolean {
  ...
  return (HOT_LEAD_CLASSIFICATIONS as readonly string[]).includes(thread.latest_classification);
}

// "All" excludes warm-up + spam. Per Dean's intent: the default useful view is
// real replies, not Snov.io noise.
export function isAllThread(thread: ThreadLike, knownSenders: Set<string>): boolean {
  return !isWarmUpThread(thread) && !isSpamThread(thread, knownSenders);
}
...
// (matchesTab) tightens the result for spam's known-sender check.
...
export function postgrestHintsFor(tab: Tab): PostgrestTabHints {
  switch (tab) {
    ...
    case 'all':
    default:
      return { subjectNotIlike: `%${WARMUP_SUBJECT_MARKER}%` };
  }
}
