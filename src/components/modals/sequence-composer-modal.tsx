...
//
// CC #UI-2 (2026-05-02): extended for subsequence creation via existing-but-
// dead-code <SubsequenceTriggerEditor> (206 LOC). When sequenceType is
...
// required + primary uniqueness + subsequence trigger fields) and
// .../sequences/[seqId]/route.ts (PATCH).
//
...
function initialTriggerConfig(existing?: CampaignSequence): SubsequenceTriggerConfig {
  return {
    trigger_event: existing?.trigger_event ?? "Reply Classified",
    trigger_condition: (existing?.trigger_condition as Record<string, unknown>) ?? { classification: "INTERESTED" },
    trigger_priority: existing?.trigger_priority ?? 1,
...
export default function SequenceComposerModal({
  ...
  const effectiveType: SequenceType =
    existingSequence?.sequence_type === "subsequence"
      ? "subsequence"
      : existingSequence?.sequence_type === "primary"
      ? "primary"
      : sequenceType ?? "primary";
  const isSubsequence = effectiveType === "subsequence";
  ...
  const titleText = (() => {
    ...
    if (mode === "create") return "New Primary Sequence";
    if (isSubsequence) return "Edit Subsequence";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const errors = validateComposerInput(
      { name, persona, steps },
      effectiveType,
      isSubsequence ? triggerConfig : null
    );
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <input
                ...
                placeholder={isSubsequence ? "e.g. INTERESTED reply follow-up" : "e.g. Atlanta dentist intro v3"}
              />
              {fieldErrors.name && (
            ...
            <div className="text-xs text-gray-500">
              Sequence type: <span className="text-gray-300 font-mono">{effectiveType}</span>
              {!isSubsequence && " — subsequences are authored separately."}
            </div>

            {isSubsequence && (
              <div className="border-t border-gray-800 pt-4">
                <SubsequenceTriggerEditor
                  trigger_event={triggerConfig.trigger_event}
                  trigger_condition={triggerConfig.trigger_condition}
                  trigger_priority={triggerConfig.trigger_priority}
                  persona={triggerConfig.persona}
                  onChange={(config) => setTriggerConfig(config)}
                />
                {fieldErrors.trigger_event && (
                  <p className="text-red-400 text-xs mt-2">{fieldErrors.trigger_event}</p>
                )}
                {fieldErrors.trigger_condition && (
                  <p className="text-red-400 text-xs mt-1">{fieldErrors.trigger_condition}</p>
                )}
                {fieldErrors.trigger_priority && (
                  <p className="text-red-400 text-xs mt-1">{fieldErrors.trigger_priority}</p>
                )}
                {fieldErrors.persona && (
                  <p className="text-red-400 text-xs mt-1">{fieldErrors.persona}</p>
                )}
